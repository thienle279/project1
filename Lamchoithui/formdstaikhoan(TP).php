...
$sql1 = "SELECT COUNT(matk) as slnv from taikhoan tk, nhanvien nv  where nv.manv = tk.manv and nv.mapb='$mapb' and tk.chucvu = 'Nhân Viên'";
...
$sql2 = "SELECT COUNT(matk) as slnv2 from taikhoan tk, nhanvien nv  where nv.manv = tk.manv and nv.mapb='$mapb'and tk.trangthai = 'Đang Làm Việc' and tk.chucvu = 'Nhân Viên'";
...
<div class="table-container"> <table id="tktable">
        <thead>
            <tr>
                <th>Mã Nhân Viên</th>
                <th>Họ Tên</th>
...
<script>
    ...
    function fetchtkNhanVienData() {
        ...
        if (phongbanId && trangthai) {
            fetch(`get_taikhoan1.php?mapb=${phongbanId}&trangthai=${trangthai}`)
                .then(response => {
                    if (!response.ok) {
    ...
    const phongbanId = phongbanSelect.value; // Lấy giá trị mã phòng ban từ dropdown
    ...
    if (tenNhanVien === "" ) {
        ...
        // Xây dựng URL với cả `tennv` và `mapb`
        const url = `timkiem_taikhoan1.php?tennv=${encodeURIComponent(tenNhanVien)}&mapb=${encodeURIComponent(phongbanId)}`;
        ...
        fetch(url)
            ...
            .then(data => {
                // Reset dropdown trạng thái về mặc định nếu cần
